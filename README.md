...
This is a mod for [Factorio](http://factorio.com) that implements a type of "Linked pipe". It adds 2 pump entities - Unipipe Filler and Unipipe Extractor - which fill or extract from a per-fluid globally shared fluid storage, one per fluid type.
...
Once placed, connect a Filler pump to a fluid network to fill the global storage with the connected fluid type. Elsewhere, connect an Extractor pump to extract that fluid from global storage.
...
Fluid temperatures are not preserved by the Unipipe Extractors, because the fluid-to-item conversion process loses the temperature information. This is solvable on a case-by-case basis, but I can't think of a general case solution. In any case, Unipipe thus far makes no attempt to address this issue, so fluids are yielded with their default temperatures. (This means you can't transport 500 degree steam.)